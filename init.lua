...
vim.o.updatetime = 500 --show hover diagnistic after this time
...
require('lazy').setup({
...

  {
    ...
    config = function()
      ...
      -- Optional: Grep current word
      vim.keymap.set('n', '<leader>fw', function()
        fzf.grep_cword { prompt = 'Grep Current Word > ' }
      end, { desc = '(f)ind current (w)ord (fzf-lua)' })

      -- Optional: Grep string input
      vim.keymap.set('n', '<leader>fs', function()
        fzf.grep { prompt = 'Grep > ' }
      end, { desc = '(f)ind input (s)tring' })

      -- Optional: Command history
      vim.keymap.set('n', '<leader>:', fzf.command_history, { desc = 'Command History(:)' })

      -- Optional: Search in current buffer
      vim.keymap.set('n', '<leader>f/', fzf.blines, { desc = '(f)inde buffer lines(/)' })
    end,
  },
  ...
  },
  {
    ...
    config = function()
      ...
      --  This function gets run when an LSP agtaches to a particular buffer.
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('gW', fzf.lsp_workspace_symbols, 'Open (W)orkspace Symbols')
